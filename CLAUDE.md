...
src/
  ...
  tools.ts    — MCP tool definitions (context, read, outline, read section, frontmatter, links, create/update/edit/edit section/trash, set frontmatter, search title, search content, daily note, feedback)
  lock.ts     — Per-path async mutex; serializes read-modify-write so concurrent edits to one note can't interleave
  merge.ts    — Pure line-based three-way (diff3) merge used to combine concurrent edits to a note
...
- **Concurrent edits to the same note** are handled in two layers. (1) A per-path async mutex (`lock.ts`, `withPathLock`) wraps every read-modify-write (`editNoteSection`, `setFrontmatterProperty`, `prependToNote`, `replaceInNote`, `updateNote`) so two tool calls touching the same note can't interleave their read and write. Different notes never block each other; `appendNote` needs no lock because `O_APPEND` is atomic. (2) `vault_read` returns a content-addressed `version` and remembers the text behind it (bounded FIFO cache, ~200 entries). `vault_update` accepts that version as `base_version`: if the note changed since the caller read it, it three-way merges the caller's content against the current file using the remembered base (`merge.ts`). Non-overlapping concurrent edits both land automatically; overlapping edits, an evicted base, or a note over `MERGE_MAX_LINES` are rejected with a `ConcurrentEditError` ("re-read and reapply") rather than silently overwriting. Omitting `base_version` keeps the old last-writer-wins overwrite.
